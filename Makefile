# build
CC		 				:= clang
CFLAGS 				:= -g -I./c -Wall -Werror -O2 -fPIC
...
DEPS := arrays.h assert.h constants.h diff.h log.h luabridge.h patch.h segments.h show.h words.h
...
c_test: $(OBJ_C_TEST)
	$(CC) -g -o c_test $(OBJ_C_TEST) `pkg-config --libs lua-5.1 libgit2`
