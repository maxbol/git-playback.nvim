...
void diff_match_lines(gplayback_text *outer, gplayback_text *inner) {
  ...
  unsigned int outer_line_count = outer_words->line_count;
  unsigned int inner_line_count = inner_words->line_count;

  assert(outer_line_count <= WORD_MAX_LINES, "Too many lines in outer text");
  assert(inner_line_count <= WORD_MAX_LINES, "Too many lines in inner text");

  char *outer_line_char =
      malloc(sizeof(char) * WORD_LINE_MAX_LEN * outer_line_count);
  char *inner_line_char =
      malloc(sizeof(char) * WORD_LINE_MAX_LEN * inner_line_count);

  gplayback_word_id *outer_line_anchor =
      malloc(sizeof(gplayback_word_id) * outer_line_count);
  gplayback_word_id *inner_line_anchor =
      malloc(sizeof(gplayback_word_id) * inner_line_count);

  unsigned int *outer_line_size =
      malloc(sizeof(unsigned int) * outer_line_count);
  unsigned int *inner_line_size =
      malloc(sizeof(unsigned int) * inner_line_count);
  ...
  while (outer_cursor != 0) {
    ...
    outer_line_size[outer_line_offset] = words_print_line(
        &outer_line_char[outer_line_offset * WORD_LINE_MAX_LEN],
        WORD_LINE_MAX_LEN, outer_words, outer_cursor);
  ...
  while (inner_cursor != 0) {
    ...
    inner_line_size[inner_line_offset] = words_print_line(
        &inner_line_char[inner_line_offset * WORD_LINE_MAX_LEN],
        WORD_LINE_MAX_LEN, inner_words, inner_cursor);
  ...
  for (int i = 0; i < outer_line_offset; i++) {
    ...
    char *outer_line = &outer_line_char[i * WORD_LINE_MAX_LEN];

    for (int j = 0; j < inner_line_offset; j++) {
      unsigned int inner_size = inner_line_size[j];
      char *inner_line = &inner_line_char[j * WORD_LINE_MAX_LEN];
...
void diff_match_filler(
    gplayback_text *outer, gplayback_text *inner,
    bool (*match_heuristic)(gplayback_word_list_entry *entry,
                            gplayback_word_list_entry *last_matched_entry)) {
  gplayback_word_list *outer_words = &outer->words;
  gplayback_word_list *inner_words = &inner->words;

  gplayback_word_id outer_cursor = outer_words->first;
  gplayback_word_list_entry *outer_last_matched_entry = NULL;
  ...
  while (outer_cursor != 0) {
    ...
    if (outer_entry->item.match != 0) {
      outer_last_matched_entry = outer_entry;
    } else if (match_heuristic(outer_entry, outer_last_matched_entry)) {
      gplayback_word_id inner_cursor =
          outer_last_matched_entry == NULL
              ? inner_words->first
              : outer_last_matched_entry->item.match;
...
bool diff_wmatch_newlines(gplayback_word_list_entry *entry,
                          gplayback_word_list_entry *last_matched_entry) {
  if (!words_is_linesep(entry->item)) {
    return false;
  }

  if (entry->item.col_idx == 0) {
    return true;
  }

  if (last_matched_entry == NULL ||
      last_matched_entry->item.line_idx == entry->item.line_idx) {
    return true;
  }
...
bool diff_wmatch_whitespaces(gplayback_word_list_entry *entry,
                             gplayback_word_list_entry *last_matched_entry) {
  return words_is_whitespace(entry->item);
}

void diff_match_newlines(gplayback_text *outer, gplayback_text *inner) {
  return diff_match_filler(outer, inner, diff_wmatch_newlines);
}

void diff_match_whitespaces(gplayback_text *outer, gplayback_text *inner) {
  return diff_match_filler(outer, inner, diff_wmatch_whitespaces);
}
